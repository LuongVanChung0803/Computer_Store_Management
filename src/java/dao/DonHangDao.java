...
public class DonHangDao {
   ...
   // Lấy danh sách đơn hàng của một người dùng cụ thể
    ...
    public void addDonHang(int id_khachhang,int id_sanpham ,int so_luong,Date ngay_dat, String trang_thai,String hinh_thuc_thanh_toan, float tong_tien) {
     ...
     // Kiểm tra id_donhang có tồn tại không
    public static boolean isIdDonHangExists(int id_donhang) {
        String sql = "SELECT COUNT(*) FROM DonHang WHERE id_donhang = ?";
    ...
    public void addDonHang_vnpay(int id_donhang ,int id_khachhang,int id_sanpham ,int so_luong,Date ngay_dat, String trang_thai,String hinh_thuc_thanh_toan, float tong_tien) {
        String query = "INSERT INTO DonHang (id_donhang,id_khachhang,id_sanpham,so_luong, ngay_dat, trang_thai,hinh_thuc_thanh_toan, tong_tien) VALUES (?,?, ?, ?, ?,?,?,?)";
        try {
            conn = connectionDB.getConnection();
            ps = conn.prepareStatement(query);
            ps.setInt(1, id_donhang);
            ps.setInt(2, id_khachhang);
             ps.setInt(3, id_sanpham);
              ps.setInt(4, so_luong);
      
            ps.setDate(5, new java.sql.Date(ngay_dat.getTime()));
            ps.setString(6, trang_thai);
             ps.setString(7, hinh_thuc_thanh_toan);
            
            ps.setFloat(8, tong_tien);
            ps.executeUpdate();
        } catch (SQLException e) {
        } finally {
            closeResources();
   ...
   //delete
    public void deleteoder_ad(String id_donhang) {
        String query = "DELETE FROM DonHang WHERE id_donhang = ?";
        try {
            conn = connectionDB.getConnection();
            ps = conn.prepareStatement(query);
            ps.setString(1, id_donhang);
            ps.executeUpdate();
        } catch (SQLException e) {
        } finally {
            closeResources();
 ...
 // timkiemdonhang
 public List<DonHang> searchOrdersByCustomerName(String customerName) {
        List<DonHang> orders = new ArrayList<>();
        String query = "SELECT * FROM DonHang where hinh_thuc_thanh_toan LIKE ?";
        try {
            conn = connectionDB.getConnection();
            ps = conn.prepareStatement(query);
